...
The tuning process is facilitated by setting the parameter OUTFIL to 4, CBFREQ to 0, NODELIM to 0, and HEURIST e.g. to 1. All the single-scenario problems for dual bounds (lb$\ldots$) and primal bounds (ub$\ldots$) are then written to the directory sipout for use in determining good CPLEX settings.
...
BESTFR&Int&1..&30&frequency for choosing the best bound node regardless of the other criteria\\
...
The default is the value 100, which applies all heuristics 1 to 10 in every node plus heuristic 12 (test all scenario solutions) in the first 11 nodes,
