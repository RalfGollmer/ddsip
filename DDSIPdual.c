            ...
            if (DDSIP_bb->cutAdded)
            {
                ...
                do
                {
                    ...
                    if (!DDSIP_killsignal)
                    {
                        ...
                        break;
                } while (DDSIP_bb->cutAdded && (((obj - old_obj)/(fabs(obj)+1e-16) > 4.e-12) || (noIncreaseCounter < 4))
...
//######################
                            ...
                            (DDSIP_bb->bestvalue < DDSIP_infty && (obj - DDSIP_bb->bestbound) > 0.95*(DDSIP_bb->bestvalue - DDSIP_bb->bestbound)))
                        {
                            ...
                            old_obj = obj;
                            if (DDSIP_Dmax (obj, max_bound) >=  DDSIP_bb->bestvalue - 0.5*(fabs(DDSIP_bb->bestvalue) + 1e-9)*DDSIP_param->relgap - 1e-9)
                                cnt = 0;
...
//////////////////////////
    ...
    else
    {
        ...
        if (DDSIP_bb->curnode >= DDSIP_param->cbBreakIters)
            current_itlim = DDSIP_param->cbitlim;
        else if (!DDSIP_bb->curnode)
...
NEXT_TRY:
            cb_set_inner_update_limit (p, DDSIP_param->cb_maxsteps + (DDSIP_bb->dualdescitcnt==1?5:0) + ((DDSIP_bb->curnode < 5)?4:0) + 2);
...
//######################
        if (DDSIP_param->outlev)
        {
            ...
            {
                if (DDSIP_bb->dualdescitcnt >= current_itlim)
