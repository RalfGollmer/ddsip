...
DDSIP_SkipUB (void)
{
    ...
    if (DDSIP_node[DDSIP_bb->curnode]->bound > DDSIP_bb->bestvalue + DDSIP_param->accuracy*fabs(DDSIP_bb->bestvalue))
        status = 2;
...
DDSIP_Warm (int iscen)
{
    ...
    if (DDSIP_bb->DDSIP_step == dual && DDSIP_bb->dualitcnt)
    {
        ...
        status = CPXaddmipstarts (env, lp, 1, DDSIP_bb->total_int, DDSIP_bb->beg, DDSIP_bb->intind, DDSIP_bb->intsolvals[scen], DDSIP_bb->effort, DDSIP_bb->Names);
        if (status)
        {
    ...
    else if (DDSIP_param->hot == 1)
    {
        ...
        if (DDSIP_bb->curnode)
        {
            ...
            status = CPXaddmipstarts (env, lp, 1, DDSIP_bb->total_int, DDSIP_bb->beg, DDSIP_bb->intind, DDSIP_bb->values, DDSIP_bb->effort, DDSIP_bb->Names);
            status = CPXsetintparam (env, CPX_PARAM_ADVIND, 2);
            if (status)
            {
                fprintf (stderr, "ERROR: Failed to set cplex parameter CPX_PARAM_ADVIND.\n");
    ...
    else if (DDSIP_param->hot == 4 || DDSIP_param->hot == 6)
    {
        ...
        if (added)
        {
            //status = CPXaddmipstarts (env, lp, added, added*DDSIP_bb->total_int, DDSIP_bb->beg, DDSIP_bb->indices, DDSIP_bb->values, DDSIP_bb->effort, DDSIP_bb->Names);
            for (j = 0; j < added; j++)
            {
                if (DDSIP_param->outlev > 99)
                {
                ...
                status = CPXaddmipstarts (env, lp, 1, DDSIP_bb->total_int, DDSIP_bb->beg, DDSIP_bb->intind, DDSIP_bb->values + j*DDSIP_bb->total_int, DDSIP_bb->effort, DDSIP_bb->Names + j);
                if (status)
                {
    ...
    if (DDSIP_bb->curnode && (DDSIP_param->hot == 2 || DDSIP_param->hot > 4) && !DDSIP_param->riskmod && !DDSIP_param->cb)
    {
        ...
        status = CPXchgrhs (env, lp, 1, rhsind, values);
        if (status)
        {
...
DDSIP_LowerBoundWriteLp (int scen)
{
    ...
    else if(DDSIP_param->files == 5)
    {
        ...
        if (CPXgetnummipstarts(env,lp))
        {
            sprintf (fname, "%s/lb_sc%d_n%d%s", DDSIP_outdir, scen + 1, DDSIP_bb->curnode, ".mst.gz");
            status = CPXwritemipstarts(env,lp,fname,0, CPXgetnummipstarts(env,lp)-1);
            if (status)
            {
        ...
        status = CPXwriteparam(env,fname);
        if (status)
        {
    ...
    else if(DDSIP_param->files == 6)
    {
        ...
        if (CPXgetnummipstarts(env,lp))
        {
            sprintf (fname, "%s/lb_sc%d_n%d_gc%d%s", DDSIP_outdir, scen + 1, DDSIP_bb->curnode, DDSIP_bb->lboutcnt-1, ".mst");
            status = CPXwritemipstarts(env,lp,fname,0, CPXgetnummipstarts(env,lp)-1);
            if (status)
            {
                fprintf (stderr, "ERROR: Failed to write startinfo\n");
...
DDSIP_LowerBound (void)
{
    ...
    if (relax == 1)
    {
        ...
        status = CPXchgctype (env, lp, DDSIP_bb->firstvar, DDSIP_bb->firstindex, type);
        if (status)
        {
...
//
        ...
        if ((DDSIP_node[DDSIP_bb->curnode]->first_sol)[scen])
        {
            ...
            if (rest_bound > -DDSIP_infty /*&& DDSIP_param->prematureStop*/ && !DDSIP_param->riskalg && iscen < DDSIP_param->scenarios-1 && ((tmpbestbound + rest_bound) > (DDSIP_bb->bestvalue*factor)) && !DDSIP_param->scalarization)
            {
                ...
                if (DDSIP_param->outlev > 90)
                {
                    fprintf(DDSIP_bb->moreoutfile," --premature stop with tmpbestbound + rest_bound: %18.14g + %18.14g = %18.14g\n", tmpbestbound, rest_bound, DDSIP_node[DDSIP_bb->curnode]->bound);
        ...
        else
        {
            ...
            if (relax < 2)
            {
                ...
                if (!optstatus && !DDSIP_Error(optstatus) && !DDSIP_Infeasible (mipstatus))
                {
                    ...
                    if (DDSIP_param->cpxscr || DDSIP_param->outlev > 10)
                    {
                        ...
                        if (DDSIP_param->outlev)
                            fprintf (DDSIP_bb->moreoutfile,"      LB: after 1st optimization: mipgap %% %-12lg %5d nodes  (%6.2fs)\n",mipgap*100.0,j,time_lap-time_start);
                    }
                        ...
                        if (mipgap > wr)
                        {
                            ...
                            if (DDSIP_param->cpxscr || DDSIP_param->outlev > 10)
                            {
                                ...
                                else
                                {
                                    ...
                                    printf ("      LB: after 2nd optimization: mipgap %% %-12lg %5d nodes  (%6.2fs)\n",mipgap*100.0,j,time_end-time_lap);
                                    if (DDSIP_param->outlev)
                                        fprintf (DDSIP_bb->moreoutfile,"      LB: after 2nd optimization: mipgap %% %-12lg %5d nodes  (%6.2fs)\n",mipgap*100.0,j,time_end-time_lap);
                                }
                            }
                        }
                        if (DDSIP_param->watchkappa)
                ...
                if ((k = CPXgetnummipstarts(env, lp)) > 2)
                {
                    status    = CPXdelmipstarts (env, lp, 1, k-1);
                }
            }
            else
            {
                // Optimize relaxed MIP
                optstatus = CPXdualopt (env, lp);
                mipstatus = CPXgetstat (env, lp);
            }
            ...
            if (DDSIP_Error (optstatus))
            {
                ...
                {
                    optstatus = CPXgetsubstat(env, lp);
            ...
            if (mipstatus==CPXMIP_TIME_LIM_INFEAS)
            {
                ...
                CPXgetdblparam (env,CPX_PARAM_TILIM,&cpu_secs);
            ...
            if(DDSIP_param->cb)
            {
                ...
                else
                {
                    ...
                    {
                        fprintf(DDSIP_bb->moreoutfile,"  ---  subbound with Lagrangean part: (subbound[%d]) %18.14g ", scen, (DDSIP_node[DDSIP_bb->curnode]->subbound)[scen]);
                    }
                    //
                    ...
                    if (DDSIP_param->outlev > 80)
                    {
                        fprintf(DDSIP_bb->moreoutfile,"without: %18.14g, difference=%g", wr, wrtmp-wr);
                    }
                    //
                    if (wrtmp < wr)
                        wr = wrtmp;
                    ...
                    if (DDSIP_param->outlev > 80)
                    {
                        fprintf(DDSIP_bb->moreoutfile," -> chosen subbound: %18.14g\n", wr);
                    }
                    //
                }
...
//        }
            ...
            if (DDSIP_bb->solstat[scen])
            {
                ...
                for (j = 0; j < DDSIP_bb->secvar; j++)
                {
                    ...
                    if (DDSIP_param->outlev >= DDSIP_second_stage_outlev)
                    {
                        fprintf (DDSIP_bb->moreoutfile, "%18.14g\t",  mipx[DDSIP_bb->secondindex[j]]);
                        if (!((j + 1) % 5))
                            fprintf (DDSIP_bb->moreoutfile, "\n");
                    }
                }
    ...
    if (!(DDSIP_bb->curnode))
    {
        ...
        {
            // debug output
            fprintf (DDSIP_bb->moreoutfile,"order of scenarios after sorting\n");
            for (iscen = 0; iscen < DDSIP_param->scenarios; iscen++)
            {
                fprintf(DDSIP_bb->moreoutfile," %3d: Scen%3d  %g =  %g * %g\n",
                        iscen+1, DDSIP_bb->lb_scen_order[iscen], sort_array[DDSIP_bb->lb_scen_order[iscen]],
        ...
        if (!DDSIP_param->riskalg && !DDSIP_param->scalarization)
        {
            ...
            else if (abs(DDSIP_param->riskmod) == 5)
            {
                ...
                {
//                  printf ("\n     CBWEIGHT less than 0.03*(lb of TVaR), resetting to %g, CBFACTOR to %g.\n\n", tmp, 1e-1/tmp);
...
//    if (DDSIP_param->cb || DDSIP_param->scalarization) {
//      tmpbestbound = DDSIP_RiskLb (DDSIP_node[DDSIP_bb->curnode]->ref_scenobj);
//      if (DDSIP_param->outlev > 50)
//        fprintf (DDSIP_bb->moreoutfile, "      tmpbestbound set by DDSIP_RiskLb to %g\n", tmpbestbound);
//    }
        else
    ...
    if (DDSIP_bb->skip)
    {
        ...
        DDSIP_bb->violations = DDSIP_bb->firstvar;
        //DDSIP_node[DDSIP_bb->curnode]->dispnorm = DDSIP_infty;
        DDSIP_node[DDSIP_bb->curnode]->dispnorm = -DDSIP_infty;
    }
    else
    {
...
DDSIP_CBLowerBound (double *objective_val, double relprec)
{
    ...
    //   if ((status = CPXgetdblparam (env, CPX_PARAM_EPGAP, &cplexRelGap))) {
    ...
    //       if ((status = CPXsetdblparam (env, CPX_PARAM_EPGAP, relprec))) {
        ...
        if ((DDSIP_node[DDSIP_bb->curnode]->first_sol)[scen] == NULL)
        {
            ...
            if (relax < 2)
            {
                ...
                if (!optstatus && !DDSIP_Error(optstatus) && !DDSIP_Infeasible (mipstatus))
                {
                    ...
                    {
                        j = CPXgetnodecnt (env,lp);
                        printf ("      CBLB: after 1st optimization: mipgap %% %-12lg %5d nodes  (%6.2fs)\n",mipgap*100.0,j,time_lap-time_start);
                        if (DDSIP_param->outlev)
                            fprintf (DDSIP_bb->moreoutfile,"      CBLB: after 1st optimization: mipgap %% %-12lg %5d nodes  (%6.2fs)\n",mipgap*100.0,j,time_lap-time_start);
                    }
                    if (DDSIP_param->watchkappa)
                    {
                        double maxkappaval, stablekappaval, suspiciouskappaval, unstablekappaval, illposedkappaval;
                        ...
                        if (mipgap > wr)
                        {
                            ...
                            if (DDSIP_param->cpxscr || DDSIP_param->outlev > 10)
                            {
                                ...
                                else
                                {
                                    j = CPXgetnodecnt (env,lp);
                                    time_end = DDSIP_GetCpuTime ();
                                    printf ("      CBLB: after 2nd optimization: mipgap %% %-12lg %5d nodes  (%6.2fs)\n",mipgap*100.0,j,time_end-time_lap);
                                    if (DDSIP_param->outlev)
                                        fprintf (DDSIP_bb->moreoutfile,"      CBLB: after 2nd optimization: mipgap %% %-12lg %5d nodes  (%6.2fs)\n",mipgap*100.0,j,time_end-time_lap);
                                }
                            }
                        }
                        if (DDSIP_param->watchkappa)
            ...
            else
            {
                ...
                else			// Solution exists
                {
                    ...
                    //
                    //            if(DDSIP_param->outlev > 90)
                    //              fprintf(DDSIP_bb->moreoutfile," after Numerical    objval= %-18.14g, bobjval= %-18.14g\n",objval,bobjval);
                    //
        ...
        for (j = 0; j < DDSIP_param->scenarios; j++)
        {
            if (!DDSIP_bb->bestfirst[j].first_sol)
            {
                ...
                if (DDSIP_param->outlev > 69)
                {
                    ...
                    {
                        fprintf (DDSIP_bb->moreoutfile," %18.14g,", DDSIP_bb->bestfirst[j].first_sol[k1]);
                        if (!((k1 + 1) % 5))
                            fprintf (DDSIP_bb->moreoutfile, "\n");
                    }
                    fprintf (DDSIP_bb->moreoutfile, "\n");
