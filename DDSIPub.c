...
DDSIP_UpperBound (int nrScenarios, int feasCheckOnly)
{
        ...
        // Print output
    {
        ...
        for (j = 0; j < DDSIP_bb->firstvar; j++)
        {
            if (DDSIP_bb->firsttype[j] == 'B' || DDSIP_bb->firsttype[j] == 'I' || DDSIP_bb->firsttype[j] == 'N')
            {
                ...
                fprintf (DDSIP_bb->moreoutfile, " %20.16g,", (DDSIP_bb->sug[DDSIP_param->nodelim + 2]->firstval)[j]);
    ...
    for(j=0; j<DDSIP_data->firstvar; j++)
    {
        if (DDSIP_bb->firsttype[j] == 'B' || DDSIP_bb->firsttype[j] == 'I' || DDSIP_bb->firsttype[j] == 'N')
            ...
            values[j] = DDSIP_Dmin(DDSIP_bb->uborg[j],(DDSIP_bb->sug[DDSIP_param->nodelim + 2])->firstval[j]);
        for (k = 0; k < DDSIP_bb->curbdcnt; k++)
        {
            if (DDSIP_bb->curind[k] == j)
                values[j] = DDSIP_Dmin (values[j], DDSIP_bb->curub[k]);
    ...
    for (iscen = 0; iscen < nr; iscen++)
    {
            ...
            if (!DDSIP_Infeasible (mipstatus))
            {
                    ...
                    !DDSIP_param->scalarization && DDSIP_bb->DDSIP_step != dual)
                {
                    ...
                    if (tmpbestvalue + DDSIP_param->riskweight*tmprisk + bobjval * DDSIP_data->prob[scen] + rest_bound > DDSIP_bb->bestvalue + DDSIP_param->accuracy)
                    {
...
#ifdef DEBUG
                            if (DDSIP_param->outlev)
                                fprintf (DDSIP_bb->moreoutfile, "### max time %g, mean %g, max>8.5*mean: %d ###\n", cpu_secs, wall_secs, cpu_secs > 8.5*wall_secs);
#endif
                            if (cpu_secs > 8.5*wall_secs)
                            {
                                cpu_hrs = 0;
                                for (wall_hrs = DDSIP_bb->shifts; wall_hrs <= iscen-cpu_hrs; wall_hrs++)
                                {
                                    if (sort_array[wall_hrs] > 8.5*wall_secs)
                                    {
                        ...
                        if (DDSIP_param->addBendersCuts && DDSIP_param->testOtherScens && DDSIP_param->heuristic > 3 && DDSIP_bb->curnode < 8)
                        {
                            time_lap = DDSIP_GetCpuTime ();
        ...
        if (DDSIP_NoSolution (mipstatus))
        {
            ...
            if (DDSIP_bb->DDSIP_step != adv && DDSIP_bb->DDSIP_step != eev &&
                (DDSIP_param->addBendersCuts && (DDSIP_param->heuristic > 3 || iscen)))
            {
                CPXLPptr     DDSIP_dual_lp  = NULL;
                cutpool_t * newCut;
                int beg, end;
                ...
                {
                    beg = iscen;
                    end = (DDSIP_param->addBendersCuts > 1 || (DDSIP_bb->curnode < 8 && DDSIP_param->testOtherScens)) ? DDSIP_param->scenarios : DDSIP_Imax(iscen+1, DDSIP_bb->shifts);
                }
...
#ifdef DEBUG
                    if (DDSIP_param->outlev)
                        fprintf (DDSIP_bb->moreoutfile, "### max time %g, mean %g, max>4.2*mean: %d ###\n", cpu_secs, wall_secs, cpu_secs > 4.2*wall_secs);
#endif
                    if (cpu_secs > 4.2*wall_secs)
                    {
                        cpu_hrs = 0;
                        for (wall_hrs = DDSIP_bb->shifts; wall_hrs <= iscen-cpu_hrs; wall_hrs++)
                        {
                            if (sort_array[wall_hrs] > 4.2*wall_secs)
                            {
                                 cpu_mins = DDSIP_bb->ub_scen_order[wall_hrs];
                                 cpu_secs = sort_array[wall_hrs];
                                 if (DDSIP_param->outlev)
...
void DDSIP_EvaluateScenarioSolutions (int* comb)
{
    ...
    if (DDSIP_param->heuristic == 100)  	// use subsequently different heuristics in the same node
    {
        ...
        else if (DDSIP_param->alwaysBendersCuts && DDSIP_bb->shifts)
        {
            DDSIP_param->heuristic = 12;
