    ...
    for (iscen = 0; iscen < nr; iscen++)
    {
        ...
        if (feasCheckOnly < 1 || (feasCheckOnly == 2 && !DDSIP_param->addBendersCuts))
        {
            ...
            if (!DDSIP_Infeasible (mipstatus))
            {
                ...
                {
                    printf ("      UB: after 1st optimization: mipgap %% %-12lg %7d nodes  (%7.2fs)\n",mipgap*100.0,nodes_1st,time_lap-time_start);
                    if (DDSIP_param->outlev)
                        fprintf (DDSIP_bb->moreoutfile,"      UB: after 1st optimization: mipgap %% %-12lg %7d nodes  (%7.2fs)\n",mipgap*100.0,nodes_1st,time_lap-time_start);
                }
#endif
                            ...
                            for (Bi = iscen + 1; Bi < DDSIP_param->scenarios; Bi++)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (status == CPX_STAT_INFEASIBLE)
                                    {
                                        ...
                                        time_lap = DDSIP_GetCpuTime ();
                                        if (!status && (viol > 2e-4) && (fabs(viol - oldviol)/viol > 1.1e-8))
                                        {
                                            int rmatbeg;
                                            ...
                                            int col_nonzeros, surplus, ii, icnt;
...
#else
                    if (timeLimit)
                        viol =  5.0 - (2.5*(iscen - DDSIP_bb->shifts))/(DDSIP_param->scenarios - DDSIP_bb->shifts + 0.1);
                    else
                        viol = 15.5 - (7.5*(iscen - DDSIP_bb->shifts))/(DDSIP_param->scenarios - DDSIP_bb->shifts + 0.1);
#endif
//#ifdef DEBUG
                    if (DDSIP_param->outlev > 20)
                        fprintf (DDSIP_bb->moreoutfile, "  ### max time %g, mean %g, max>%g*mean+1: %d (shifts: %d, timeLimit: %d)###\n", cpu_secs, wall_secs, viol, cpu_secs > viol*wall_secs + 1., DDSIP_bb->shifts, timeLimit);
//#endif
...
//#endif
                ...
                if (DDSIP_param->cpxnoub2 && status != CPXMIP_OPTIMAL && feasCheckOnly < 1)
                {
                    ...
                    if (mipgap > wr)
                    {
                        ...
                        if (DDSIP_param->cpxubscr || DDSIP_param->outlev > 11)
                        {
                            ...
                            {
                                time_end = DDSIP_GetCpuTime ();
                                printf ("      UB: after 2nd optimization: mipgap %% %-12lg %7d nodes  (%7.2fs)\n",mipgap*100.0,nodes_2nd,time_end-time_lap);
                                if (DDSIP_param->outlev)
                                    fprintf (DDSIP_bb->moreoutfile,"      UB: after 2nd optimization: mipgap %% %-12lg %7d nodes  (%7.2fs)\n",mipgap*100.0,nodes_2nd,time_end-time_lap);
                            }
                        }
#endif
                        if (DDSIP_param->watchkappa)
        ...
        if (DDSIP_NoSolution (mipstatus) || (feasCheckOnly == 2))
        {
                    ...
                    !BendersCutsChecked)
            {
                ...
                {
                    beg = iscen;
                    end = ((DDSIP_param->addBendersCuts > 1 || (DDSIP_bb->curnode < 8 && DDSIP_param->testOtherScens)) /* && (DDSIP_param->heuristic > 3) */) ? DDSIP_param->scenarios : DDSIP_Imin (DDSIP_param->scenarios, DDSIP_Imax(iscen+1, DDSIP_bb->shifts+1));
                }
...
//#endif
    ...
    if (DDSIP_bb->shifts + 2 < DDSIP_param->scenarios)
    {
        ...
        for (wall_hrs = DDSIP_bb->shifts; wall_hrs < DDSIP_param->scenarios - 1; wall_hrs++)
        {
            cpu_secs = DDSIP_Dmax (cpu_secs, sort_array[wall_hrs]);
            wall_secs += sort_array[wall_hrs];
        }
...
#else
                    if (timeLimit)
                        viol =  5.9 - (2.5*(iscen - DDSIP_bb->shifts))/(DDSIP_param->scenarios - DDSIP_bb->shifts + 0.1);
                    else
                        viol = 15.5 - (7.5*(iscen - DDSIP_bb->shifts))/(DDSIP_param->scenarios - DDSIP_bb->shifts + 0.1);
#endif
//#ifdef DEBUG
...
void DDSIP_EvaluateScenarioSolutions (int* comb)
{
    ...
    if (!(DDSIP_bb->ub_sorted))
    {
        ...
        {
            // insert the least-cost scenario after the first bb->shifts+2 scenarios - to be sure to check that for feasibility, too
            if (DDSIP_bb->shifts < DDSIP_param->scenarios - 4)
            {
                int j;
                i_scen = DDSIP_bb->ub_scen_order[DDSIP_param->scenarios - 1];
                for (j = DDSIP_param->scenarios - 1; j>DDSIP_bb->shifts + 2; j--)
                    DDSIP_bb->ub_scen_order[j] = DDSIP_bb->ub_scen_order[j-1];
                DDSIP_bb->ub_scen_order[DDSIP_bb->shifts + 2] = i_scen;
            }
        }
    ...
    else
    {
        if (DDSIP_param->heuristic >= 100)      // use subsequently different heuristics in the same node
        {
            ...
            if (!(DDSIP_param->interrupt_heur && DDSIP_bb->skip == -5) && (DDSIP_bb->heurSuccess || ((DDSIP_bb->curnode < 13 || DDSIP_node[DDSIP_bb->curnode]->depth < 4) && heur == 100) || use_heur12 || DDSIP_bb->noiter%101 > 98))
            {
