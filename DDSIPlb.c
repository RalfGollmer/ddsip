...
DDSIP_SkipUB (void)
{
    ...
    if (fabs(DDSIP_bb->bestvalue - DDSIP_node[DDSIP_bb->curnode]->bound)/(fabs(DDSIP_bb->bestvalue) + 3.e-16) < 2.e-12)
    {
        return 3;
    }
...
int
DDSIP_LowerBound (double * ret_objval, int ChangeMipGap)
{
...
//#endif
    ...
    int    *ordind  = NULL;
    double *scensol = NULL;
    double sumprob, maxdispersion, rest_bound, factor = 1.;
    char **colname;
    char *colstore;
    sug_t *tmp;

    if (DDSIP_node[DDSIP_bb->curnode]-> depth < 3)
        currentCuts = 2;
    if (!(DDSIP_bb->LBIters))
    {
    ...
    *ret_objval = -DDSIP_infty;
    ...
    for (iscen = 0; iscen < DDSIP_param->scenarios; iscen++)
    {
        ...
        if ((DDSIP_node[DDSIP_bb->curnode]->first_sol)[scen])
        {
            ...
            //if (!scen)
            //    // Initialize Warm starts in case the solution of scen 0 is inherited ??? What for?
            ...
            if (rest_bound > -DDSIP_infty /*&& DDSIP_param->prematureStop*/ && !DDSIP_param->riskalg && iscen < DDSIP_param->scenarios-1 && ((tmpbestbound + rest_bound) > (DDSIP_bb->bestvalue + DDSIP_bb->correct_bounding)) && !DDSIP_param->scalarization)
            {
...
//#ifdef DEBUG
                if (DDSIP_param->outlev > 21)
                {
                    ...
                    fprintf(DDSIP_bb->moreoutfile,"                       tmpbestbound + rest_bound: %22.14g > %22.14g = %22.14g*%g, difference: %g\n", DDSIP_node[DDSIP_bb->curnode]->bound, (DDSIP_bb->bestvalue + DDSIP_bb->correct_bounding), DDSIP_bb->bestvalue, factor, DDSIP_node[DDSIP_bb->curnode]->bound-(DDSIP_bb->bestvalue + DDSIP_bb->correct_bounding));
                }
//#endif
                //
                DDSIP_bb->skip = 2;
        ...
        else
        {
            ...
            if (relax < 2)
            {
                ...
                if (!optstatus && !DDSIP_Error(optstatus) && !DDSIP_Infeasible (mipstatus))
                {
                    ...
                    {
                        printf ("      LB: after 1st optimization: mipgap %% %-12g %7d nodes  (%7.2fs)\n",mipgap*100.0,nodes_1st,time_lap-time_start);
                        if (DDSIP_param->outlev)
                        ...
                        if (mipgap > wr)
                        {
                            ...
                            if (DDSIP_param->cpxscr || DDSIP_param->outlev > 11)
                            {
                                ...
                                else
                                {
                                    ...
                                    printf ("      LB: after 2nd optimization: mipgap %% %-12g %7d nodes  (%7.2fs)\n",mipgap*100.0,nodes_2nd,time_end-time_lap);
                                    if (DDSIP_param->outlev)
...
#ifdef SHIFT
            if (DDSIP_param->hot == 4 || (DDSIP_bb->LBIters == 1))
            {
...
// negative differences did occur - strange!...
            ...
            if (DDSIP_bb->solstat[scen])
            {
                ...
                else
                {
                    ...
                    if (DDSIP_param->addBendersCuts > 0 && (j = DDSIP_bb->cutCntr) && (DDSIP_param->numberScenReeval > -1) && currentCuts)
                    {
                        ...
                        if ((j < DDSIP_bb->cutCntr) && (nrFeasCheck <= ((DDSIP_bb->curnode < 11 || DDSIP_node[DDSIP_bb->curnode]->depth < 4) ? DDSIP_param->numberScenReeval : DDSIP_Imin(2, DDSIP_param->numberScenReeval))) && !DDSIP_killsignal)
                        {
                            if(DDSIP_param->outlev)
                            {
            ...
            // Node can be fathomed ?
            if (rest_bound > -DDSIP_infty /*&& DDSIP_param->prematureStops*/ && !DDSIP_param->riskalg && iscen < DDSIP_param->scenarios-1 && ((tmpbestbound + rest_bound) > (DDSIP_bb->bestvalue + DDSIP_bb->correct_bounding)) && !DDSIP_param->scalarization)
            {
                DDSIP_node[DDSIP_bb->curnode]->bound = tmpbestbound + rest_bound;
                //
#ifdef DEBUG
...
#ifdef GREATERMIPGAP
    if (ChangeMipGap && (DDSIP_bb->LBIters >= 2))
    {
        double cplexRelGap;
        if ((status = CPXgetdblparam (DDSIP_env, CPX_PARAM_EPGAP, &cplexRelGap)))
        {
        ...
        else
        {
            ...
            {
              fprintf (stderr, "5 reset CPLEX parameter CPX_PARAM_EPGAP from %g (LowerBound) \n", cplexRelGap);
              if (DDSIP_param->outlev)
                fprintf (DDSIP_bb->moreoutfile, "5 reset CPLEX parameter CPX_PARAM_EPGAP to %g (LowerBound) \n", cplexRelGap);
            }
        }
    }
#endif
...
//#endif
                                            ...
                                            else if (DDSIP_bb->signal)
                                            {
                                                //  user changed CPLEX screen output - reset CPLEX parameters to lb (LowerBound)
                                                status = DDSIP_SetCpxPara (DDSIP_param->cpxnolb, DDSIP_param->cpxlbisdbl, DDSIP_param->cpxlbwhich, DDSIP_param->cpxlbwhat);
                                                if (status)
                                                {
                                                    fprintf (stderr, "ERROR: Failed to set CPLEX parameters (LowerBound) \n");
...
DDSIP_CBLowerBound (double *objective_val, double relprec)
{
    ...
    static double DualObj, last_relprec = 0.;
    ...
    int nrFeasCheck = 0;
    sug_t *tmp;
    // temporary: output of relprec - does it indicate re-evaluation in center?
    printf ("### DualUpdate: relprec = %12g, last_relprec = %12g ###\n", relprec, last_relprec);
#ifdef CBHOTSTART
#ifdef SHIFT
    time_t   startTime;
...
#ifdef REDUCPLEXMIPGAP
    //
    // reduce mip gap parameter for small relprec - probably a reeval in center
    if (relprec < 2.e-2*last_relprec)
    {
        double cplexRelGap;
        if ((status = CPXgetdblparam (DDSIP_env, CPX_PARAM_EPGAP, &cplexRelGap)))
        {
        ...
        {
            if ((status = CPXsetdblparam (DDSIP_env, CPX_PARAM_EPGAP, 0.1*cplexRelGap)))
            {
            ...
            {
              fprintf (stderr, "6 reset CPLEX parameter CPX_PARAM_EPGAP from %g to %g (CBLowerBound) \n", cplexRelGap, 0.1*cplexRelGap);
              if (DDSIP_param->outlev)
                fprintf (DDSIP_bb->moreoutfile, "6 reset CPLEX parameter CPX_PARAM_EPGAP from %g to %g (CBLowerBound) \n", cplexRelGap, 0.1*cplexRelGap);
            }
        }
    }
...
NEXT_SCEN:
            ...
            if (relax < 2)
            {
                ...
                if (!optstatus && !DDSIP_Error(optstatus) && !DDSIP_Infeasible (mipstatus))
                {
                    ...
                    if (DDSIP_param->cpxscr || DDSIP_param->outlev > 11)
                    {
                        printf ("      CBLB: after 1st optimization: mipgap %% %-12g %7d nodes  (%7.2fs)\n",mipgap*100.0,nodes_1st,time_lap-time_start);
                        if (DDSIP_param->outlev)
                        {
                            if (use_LB_params)
                                fprintf (DDSIP_bb->moreoutfile,"      CBLB: after 1st optimization: mipgap %% %-12g %7d nodes  (%7.2fs), status: %d   - using CPLEX parameters for LB\n",mipgap*100.0,nodes_1st,time_lap-time_start, mipstatus);
                            else
                                fprintf (DDSIP_bb->moreoutfile,"      CBLB: after 1st optimization: mipgap %% %-12g %7d nodes  (%7.2fs), status: %d\n",mipgap*100.0,nodes_1st,time_lap-time_start, mipstatus);
                        }
                    }
#endif
                    if (DDSIP_param->watchkappa)
                    ...
                    if (mipgap > 0. && mipstatus != CPXMIP_OPTIMAL && ((use_LB_params && DDSIP_param->cpxnolb2) || (!use_LB_params && DDSIP_param->cpxnodual2)))
                    {
                        ...
                        if (relprec < 2.e-2*last_relprec)
                        {
                            cpu_secs = 0.1*wr;
                            {
                                if ((status = CPXsetdblparam (DDSIP_env, CPX_PARAM_EPGAP, cpu_secs)))
                                {
                                  fprintf (stderr, "ERROR: Failed to set CPLEX parameter CPX_PARAM_EPGAP to %g (CBLowerBound) \n", cpu_secs);
                                  if (DDSIP_param->outlev)
                                    fprintf (DDSIP_bb->moreoutfile, "ERROR: Failed to set CPLEX parameter CPX_PARAM_EPGAP to %g (CBLowerBound) \n", cpu_secs);
                                ...
                                {
                                  fprintf (stderr, "7 reset CPLEX parameter CPX_PARAM_EPGAP from %g to %g (CBLowerBound) \n", wr, cpu_secs);
                                  if (DDSIP_param->outlev)
                                    fprintf (DDSIP_bb->moreoutfile, "7 reset CPLEX parameter CPX_PARAM_EPGAP from %g to %g (CBLowerBound) \n", wr, cpu_secs);
				  wr = cpu_secs;
                                }
                            }
                        }
                        ...
                        if (mipgap > wr)
                        {
                            ...
                            if (DDSIP_param->cpxscr || DDSIP_param->outlev > 11)
                            {
                                ...
                                {
                                    time_end = DDSIP_GetCpuTime ();
                                    printf ("      CBLB: after 2nd optimization: mipgap %% %-12g %7d nodes  (%7.2fs)\n",mipgap*100.0,nodes_2nd,time_end-time_lap);
                                    if (DDSIP_param->outlev)
                                    {
                                        if (use_LB_params)
                                            fprintf (DDSIP_bb->moreoutfile,"      CBLB: after 2nd optimization: mipgap %% %-12g %7d nodes  (%7.2fs), status: %d   - using CPLEX parameters for LB\n",mipgap*100.0,nodes_2nd,time_end-time_lap, mipstatus);
                                        else
                                            fprintf (DDSIP_bb->moreoutfile,"      CBLB: after 2nd optimization: mipgap %% %-12g %7d nodes  (%7.2fs), status: %d\n",mipgap*100.0,nodes_2nd,time_end-time_lap, mipstatus);
                                    }
                                }
                            }
#endif
                        ...
                        // reset CPLEX parameters to first call
                        status = DDSIP_SetCpxPara (DDSIP_param->cpxnodual, DDSIP_param->cpxdualisdbl, DDSIP_param->cpxdualwhich, DDSIP_param->cpxdualwhat);
                        if (status)
                        {
                            fprintf (stderr, "ERROR: Failed to set CPLEX parameters (LowerBound) \n");
                        ...
                        if (relprec < 2.e-2*last_relprec)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                  fprintf (stderr, "8 reset CPLEX parameter CPX_PARAM_EPGAP from %g to %g (CBLowerBound) \n", cplexRelGap, 0.1*cplexRelGap);
                                  if (DDSIP_param->outlev)
                                    fprintf (DDSIP_bb->moreoutfile, "8 reset CPLEX parameter CPX_PARAM_EPGAP from %g to %g (CBLowerBound) \n", cplexRelGap, 0.1*cplexRelGap);
                                }
                            }
                        }
#endif
                    ...
                    {
                        //  user changed CPLEX screen output - reset CPLEX parameters to first call
                        status = DDSIP_SetCpxPara (DDSIP_param->cpxnodual, DDSIP_param->cpxdualisdbl, DDSIP_param->cpxdualwhich, DDSIP_param->cpxdualwhat);
                        if (status)
                        {
                            fprintf (stderr, "ERROR: Failed to set CPLEX parameters (LowerBound) \n");
...
//#endif
            ...
            time (&DDSIP_bb->cur_time);
#ifdef SHIFT
            if ((shift_in_cb && !nrFeasCheck) || !DDSIP_bb->dualitcnt)
            {
                // prepare for shifting difficult scenarios to the end such that they can make use of more mipstarts
...
#ifdef SHIFT
    if (shift_in_cb || !DDSIP_bb->dualitcnt)
    {
        if (DDSIP_param->outlev)
            fprintf (DDSIP_bb->moreoutfile,"SORT: shift_in_cb= %d, bb->dualitcnt= %d\n", shift_in_cb, DDSIP_bb->dualitcnt);
        DDSIP_qsort_ins_A (DDSIP_bb->aggregate_time, DDSIP_bb->lb_scen_order, 0, DDSIP_param->scenarios-1);
        DDSIP_bb->lb_sorted = 0;
        if (DDSIP_param->outlev > 21)
        {
        ...
        if (DDSIP_param->outlev)
            fprintf (DDSIP_bb->moreoutfile,"NO:   shift_in_cb= %d, bb->dualitcnt= %d\n", shift_in_cb, DDSIP_bb->dualitcnt);
#endif
...
//        }
    ...
    *objective_val = -tmpbestbound;
    ...
    else
    {
        ...
        else if (DDSIP_bb->dualdescitcnt == 1)
        {
               ...
               )
            {
                ...
                else if ((DDSIP_node[DDSIP_bb->curnode]->bound - tmpbestbound) > (0.2*fabs(DDSIP_node[DDSIP_bb->curnode]->bound) + 1.))
                {
    ...
    if (DDSIP_param->cb_increaseWeight)
    {
        ...
        {
            DDSIP_bb->constant_obj = 1;
            if (old_value < 1.e2*cb_get_last_weight(DDSIP_bb->dualProblem))
                wr =  DDSIP_Dmax (1.0e3, 10.*cb_get_last_weight(DDSIP_bb->dualProblem));
            else
                wr =  DDSIP_Dmin (1.0e1, 0.1*cb_get_last_weight(DDSIP_bb->dualProblem));
            if (DDSIP_param->outlev > 20)
        ...
        else if (nearly_constant > 4)
        {
            ...
            else if (old_value < 1.e2*cb_get_last_weight(DDSIP_bb->dualProblem))
                wr =  0.1*cb_get_last_weight(DDSIP_bb->dualProblem);
            else
                wr =  0.01*cb_get_last_weight(DDSIP_bb->dualProblem);
            if (DDSIP_param->outlev > 20)
